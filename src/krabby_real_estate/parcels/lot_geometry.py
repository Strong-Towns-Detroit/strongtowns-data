...
def screen_dimensions(
    *,
    ...
    if district not in standards:
        return ScreenResult(ScreenStatus.NOT_EVALUATED, None, None, "district_not_in_scenario")

    area = _number(area_sqft)
    width = _number(width_ft)
    standard = standards[district]
    if area is None:
        return ScreenResult(ScreenStatus.NOT_EVALUATED, None, None, "missing_or_invalid_area")
    if require_width and standard.minimum_width_ft is not None and width is None:
        return ScreenResult(ScreenStatus.NOT_EVALUATED, None, None, "missing_or_invalid_width")
