"""Geometry helpers for Detroit Base Units and assessor parcels.
...
def exterior_segments(geometry: Polygon) -> list[LineString]:
    ...
    return [LineString([start, end]) for start, end in pairwise(coordinates) if start != end]
...
def _local_street_segment(street: LineString | MultiLineString, point) -> LineString:
    ...
    for component in components:
        ...
        candidates.extend(
            LineString([start, end]) for start, end in pairwise(coordinates) if start != end
        )
...
def estimate_street_facing_edge(
    ...
    *,
    ...
    parallel = [row for row in rows if row["angle_difference"] <= parallel_tolerance_degrees]
...
def building_parcel_overlaps(
    ...
    *,
    ...
    return material[[building_id_col, parcel_id_col, "overlap_area", "building_share"]].reset_index(
        drop=True
    )
