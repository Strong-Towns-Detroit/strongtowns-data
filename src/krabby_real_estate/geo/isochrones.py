...
Ported from John Bolt's Strong Towns Detroit analysis package at commit ``6cf23a9``.
...
def load_or_build_network(
    place=None, *, point=None, dist=None, mode="drive", cache_dir="./cache", reuse_graphml=None
):
    ...
    if reuse_graphml is not None and mode == "drive" and Path(reuse_graphml).exists():
        ...
        inside = ll is not None and min_lon <= ll[0] <= max_lon and min_lat <= ll[1] <= max_lat
        if inside:
            G = Greuse
            if not _has_travel_times(G):
                G = add_travel_times(G, mode)
...
def travel_times_from(G, source_nodes, *, cutoff_s=None):
    ...
    return nx.multi_source_dijkstra_path_length(G, sources, cutoff=cutoff_s, weight="travel_time")
...
def time_matrix(G, source_nodes, target_nodes, *, cutoff_s=None):
    ...
    for s in source_nodes:
        dist = nx.single_source_dijkstra_path_length(G, s, cutoff=cutoff_s, weight="travel_time")
...
def isochrone_polygon(
    ...
    *,
    ...
    poly = (
        edges.geometry.buffer(edge_buffer_m).union_all()
...
def _drop_slivers(geom, min_area_frac):
    ...
    parts = [part for part in geom.geoms if part.area >= min_area_frac * total_area]
    if not parts:
...
def _chaikin(coords, iterations):
    ...
    for _ in range(iterations):
        ...
        for i, point in enumerate(points):
            ...
            refined.extend(
                [
                    (0.75 * point[0] + 0.25 * nxt[0], 0.75 * point[1] + 0.25 * nxt[1]),
                    (0.25 * point[0] + 0.75 * nxt[0], 0.25 * point[1] + 0.75 * nxt[1]),
                ]
            )
        points = refined
...
def site_destination_matrix(
    G, sites, destinations, *, cutoff_min=None, max_snap_m=1000.0, return_snap=False
):
    ...
    """
    cutoff_s = cutoff_min * 60 if cutoff_min else None
    site_nodes, site_snap = snap_points(
        G, sites, max_snap_m=max_snap_m, return_dist=True, label="site"
    )
    dest_nodes, dest_snap = snap_points(
        G, destinations, max_snap_m=max_snap_m, return_dist=True, label="destination"
    )
...
def rank_sites(
    ...
    *,
    ...
    sort_key = {
        ...
        "pct_within": ("pct_within_threshold", False),
    }[objective]
    # Push sites with unreached destinations toward the bottom regardless of objective.
    df = df.sort_values(by=["n_unreached", sort_key[0]], ascending=[True, sort_key[1]]).reset_index(
        drop=True
    )
    return df
