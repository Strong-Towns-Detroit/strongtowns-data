...
from krabby_real_estate.mapping.colors import (
    BOUNDARY,
    STREET,
    WATER_EDGE,
    WATER_FILL,
    ...
    *,
    ...
    if clip_to_bounds:
        clip_gdf = gpd.GeoDataFrame(
            {"geometry": [box(*ref_gdf.total_bounds)]},
            crs=target_crs,
        )
...
def make_patch_legend(ax, color_label_pairs, *, loc="lower right", alpha=1.0, **kwargs):
    ...
    **kwargs
    ...
    handles = [
        Patch(facecolor=color, label=label, alpha=alpha) for color, label in color_label_pairs
    ]
