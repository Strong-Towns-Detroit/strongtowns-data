...
Each dataset has a versioned model, immutable snapshots, and an atomic
...
1. Define its `DatasetModel` and `DataAsset`.
...
Large immutable datasets may be archived to private R2. Payloads under
`data/datasets/` are ignored by Git; manifests and promoted pointers are
versionable.
