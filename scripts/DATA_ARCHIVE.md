...
large for Git. It is part of the `strongtowns-data` model system: archive
...
model explicitly assigns an archive tier.
...
1. Create a private R2 bucket named `strongtowns-data-data` in the Strong
...
3. Put the values below in the Git-ignored `.env` or process environment:
   ...
   R2_BUCKET=strongtowns-data-data
   ```
...
```bash
git clone <repo> strongtowns-data
cd strongtowns-data
