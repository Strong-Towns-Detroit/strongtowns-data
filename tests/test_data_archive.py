...
from strongtowns_data.pipelines.archive import DataArchive, PREFIX
from strongtowns_data.models import ArchiveTier, DataAsset, DatasetModel


def archived_asset() -> DataAsset:
    return DataAsset(
        "source.example",
        Path("data/datasets/example"),
        DatasetModel("source.example", "1.0.0"),
        ArchiveTier.SOURCE,
    )
...
def setup_archive(tmp_path):
    path = tmp_path / "data/datasets/example/snapshots/one"
...
def test_archive_manifest_is_written_only_after_all_uploads(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("strongtowns_data.pipelines.archive._client", lambda: Client(fail=True))
...
def test_archive_round_trip_verifies_hashes(tmp_path, monkeypatch):
    ...
    client = Client()
    monkeypatch.setenv("R2_BUCKET", "bucket")
    monkeypatch.setattr("strongtowns_data.pipelines.archive._client", lambda: client)
    archive.push(["source"], apply=True)
    source = tmp_path / "data/datasets/example/snapshots/one/data.bin"
    source.unlink()
