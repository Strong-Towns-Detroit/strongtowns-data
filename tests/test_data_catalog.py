...
from strongtowns_data.pipelines.catalog import (
    DATABASE_ARTIFACT,
    build_query_catalog,
    query_catalog,
    read_catalog_index,
...
from strongtowns_data.pipelines.snapshots import SnapshotStore, canonical_json
from strongtowns_data.pipelines.snapshots import sha256
...
def catalog_asset() -> DataAsset:
    return DataAsset(
        "query.catalog", Path("data/datasets/query-catalog"),
        DatasetModel(
            "query.catalog", "1.0.0",
...
def catalog_system(tmp_path, monkeypatch):
    monkeypatch.setattr(
        "strongtowns_data.pipelines.snapshots.producer_state",
...
def test_catalog_materializes_geoparquet_without_spatial_extension(
    ...
    source = DataAsset(
        "source.points", Path("data/datasets/points"),
        DatasetModel(
