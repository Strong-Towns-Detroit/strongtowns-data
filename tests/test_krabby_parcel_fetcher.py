...
from strongtowns_data.parcels.fetcher import (
    _HUB_DOWNLOAD,
    _hub_download,
    fetch_csv,
    fetch_geojson,
...
class TestHubDownload:
    @patch("strongtowns_data.parcels.fetcher.requests.get")
    ...
    @patch("strongtowns_data.parcels.fetcher.time.sleep")
    @patch("strongtowns_data.parcels.fetcher.requests.get")
...
class TestFetchGeojson:
    @patch("strongtowns_data.parcels.fetcher._hub_download")
...
class TestQueryToGeojson:
    @patch("strongtowns_data.parcels.fetcher.query_parcels")
    def test_writes_geojson_file(self, mock_query, tmp_path):
        mock_query.return_value = [{"type": "Feature", "properties": {"parcel_id": "001"}}]
    ...
    def test_passes_kwargs(self, mock_query, tmp_path):
        ...
        mock_query.assert_called_once_with(where="zoning_district='R2'", max_records=10)
