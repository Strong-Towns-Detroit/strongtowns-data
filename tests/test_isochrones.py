...
from krabby_real_estate.geo.isochrones import (
    add_travel_times,
    isochrone_polygon,
    nearest_time_to_any,
    snap_points,
...
def _chain_graph(n=15, length_m=200.0, maxspeed="30 mph"):
    ...
    for i in range(n):
        for u, v in ((i, i + 1), (i + 1, i)):
            G.add_edge(u, v, key=0, length=length_m, highway="residential", maxspeed=maxspeed)
    return G
...
def test_snap_guard_warns_beyond_threshold():
    ...
    with pytest.warns(UserWarning, match="outside the network boundary"):
        _nodes2, dists2 = snap_points(G, [far], max_snap_m=1000.0, return_dist=True)
