...
def asset(name: str, path: str) -> DataAsset:
    return DataAsset(name, Path(path), MODEL)
...
def test_model_schema_hash_is_stable_across_mapping_order():
    other = DatasetModel(
        "example.rows", "1.0.0",
    ...
    assert MODEL.schema_hash == other.schema_hash


def test_new_manifest_uses_model_version(tmp_path):
    item = asset("rows", "data/datasets/rows")
    store = SnapshotStore(tmp_path)
    snapshot_id, staging = store.create_staging(item)
    metadata = write_rows(
        PipelineContext(tmp_path, "rows", {}, {}, {item.id: staging}), item.id
    )[item.id]
    manifest = store.write_manifest(item, staging, snapshot_id, metadata, [])
    assert manifest["model_version"] == MODEL.version
    assert "contract_version" not in manifest


def test_legacy_contract_version_manifest_remains_readable(tmp_path):
    item = asset("rows", "data/datasets/rows")
    store = SnapshotStore(tmp_path)
    snapshot_id, staging = store.create_staging(item)
    metadata = write_rows(
    ...
    manifest = store.write_manifest(item, staging, snapshot_id, metadata, [])
    manifest["contract_version"] = manifest.pop("model_version")
    assert validate_snapshot(item, staging, manifest)["dataset_id"] == item.id


def test_graph_orders_producers_before_consumers(tmp_path):
    raw = asset("raw", "data/datasets/raw")
    derived = asset("derived", "data/datasets/derived")
...
def test_graph_rejects_duplicate_asset_producer(tmp_path):
    duplicate = asset("same", "data/datasets/same")
...
def test_graph_rejects_unregistered_input(tmp_path):
    output = asset("output", "data/datasets/output")
...
def test_multi_output_pipeline_validates_all_outputs_before_any_promotion(tmp_path):
    one = asset("one", "data/datasets/one")
    two = asset("two", "data/datasets/two")
...
def test_legacy_import_rejects_changed_record_count(tmp_path):
    ...
    item = DataAsset(
        "legacy", Path("data/datasets/legacy"),
        DatasetModel("legacy", "1.0.0"),
...
def test_legacy_import_preflights_every_asset_before_promoting(tmp_path):
    ...
    one = DataAsset(
        "one", Path("data/datasets/one"), DatasetModel("one", "1.0.0"),
        legacy_artifacts={"raw.csv": Path("one.csv")}, legacy_counts={"records": 1},
    )
    two = DataAsset(
        "two", Path("data/datasets/two"), DatasetModel("two", "1.0.0"),
...
def test_model_rejects_unknown_enumerated_value(tmp_path):
    model = DatasetModel(
        "enum", "1.0.0", required_columns={"id": "string", "state": "string"},
        allowed_values={"state": ("ready", "blocked")}, primary_key=("id",),
        accepted_artifact="accepted.parquet",
    )
    item = DataAsset("enum", Path("data/datasets/enum"), model)
    store = SnapshotStore(tmp_path)
    snapshot_id, staging = store.create_staging(item)
...
def test_verify_recursively_checks_immutable_parent_snapshot(tmp_path):
    parent = asset("parent", "data/datasets/parent")
    child = asset("child", "data/datasets/child")
    system = DataBuildSystem(
        tmp_path,
        (
