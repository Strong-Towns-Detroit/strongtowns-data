"""Tests for strongtowns_data.mapping (colors + layers)."""
...
class TestColors:
    ...
    def test_all_hex_colors_valid(self):
        from strongtowns_data.mapping import colors
    ...
    def test_housing_category_colors_keys(self):
        from strongtowns_data.mapping.colors import HOUSING_CATEGORY_COLORS
    ...
    def test_shortage_colors_boolean_keys(self):
        from strongtowns_data.mapping.colors import SHORTAGE_COLORS, ACCESSIBILITY_COLORS
    ...
    def test_z_order_ascending(self):
        from strongtowns_data.mapping.colors import Z_WATER, Z_DATA, Z_STREETS, Z_BOUNDARY

        assert Z_WATER < Z_DATA < Z_STREETS < Z_BOUNDARY

    def test_burden_bins_sorted(self):
        from strongtowns_data.mapping.colors import BURDEN_BINS

        assert BURDEN_BINS == sorted(BURDEN_BINS)
        assert len(BURDEN_BINS) == 5

    def test_alpha_in_range(self):
        from strongtowns_data.mapping.colors import ALPHA_BLOCKGROUPS
...
class TestMakePatchLegend:
    ...
    def test_returns_legend(self):
        ...
        from strongtowns_data.mapping.layers import make_patch_legend
